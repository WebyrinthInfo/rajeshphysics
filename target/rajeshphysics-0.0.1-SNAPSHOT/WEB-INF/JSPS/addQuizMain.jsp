...
<script>
    $(document).ready(function() {
        ...
        $('#addQuizBtn').click(function(event) {
            ...
            var maxMarks = $('#maxMarks').val().trim();
            var numberOfQuestions = $('#numberOfQuestions').val().trim();
            var description = $('#description').val().trim();
            var isActive = $('#isActive').val();
            var courseId = $('#course').val();

            // Form validation
            if (title === '' || maxMarks === '' || numberOfQuestions === '' || description === '' || isActive === '' || courseId === '') {
                Swal.fire({
                    icon: 'info',
                    title: 'Info',
            ...
            // AJAX request
            $.ajax({
                url: "${pageContext.request.contextPath}/api/quiz/save?courseId="+courseId,
                method: "POST",
                ...
                error: function(xhr, status, error) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Failed to add quiz. Please try again later.'
                    });
                }
            });
        });
